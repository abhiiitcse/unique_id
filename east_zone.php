...
<i><h2 id="east"><blink>WELCOME TO EAST ZONE</blink></h2></i>
...
if(isset($_POST['submit']))
{
	...
	$home_url = 'http://' . $row2['ipaddr'] .  '/unique_id/south_zone.php';
		...
		elseif((($unique_id <= $row3['lastrange1'])&&($unique_id > $row3['startrange1']))||(($unique_id <= $row3['lastrange2'])&&($unique_id > $row3['startrange2']))||(($unique_id <= $row3['lastrange3'])&&($unique_id > $row3['startrange3']))||(($unique_id <= $row3['lastrange4'])&&($unique_id > $row3['startrange4'])))
		
		//elseif(($unique_id < $row4['datarange']) &&(($unique_id > $row3['datarange'])|| ($unique_id = $row3['datarange'])))
		{
			//process the query
