...
<i><h2 id="west"><blink>WELCOME TO WEST ZONE</blink></h2></i>
...
if(isset($_POST['submit']))
{
	...
	if(!empty($unique_id))
	{
		...
		{
			//echo '<a href="http://' . $row1['ipaddr'] . '/uniqueid_modern/search1.php ">go to this server1</a>';
		$home_url = 'http://' . $row1['ipaddr'] .  '/unique_id/north_zone.php';
		...
		//else if($unique_id < $row3['datarange'] && (($unique_id > $row2['datarange'])||($unique_id = $row2['datarange'])))
		{
			//echo '<a href="http://' . $row2['ipaddr'] . '/uniqueid_modern/search2.php ">go to this server2</a>';
		$home_url = 'http://' . $row2['ipaddr'] .  '/uniqueid_modern/south_zone.php';
          header('Location: ' . $home_url);
		}
		...
		$home_url = 'http://' . $row3['ipaddr'] .  '/uniqueid_modern/east_zone.php';
          header('Location: ' . $home_url);
		}
		
		elseif((($unique_id <= $row4['lastrange1'])&&($unique_id > $row4['startrange1']))||(($unique_id <= $row4['lastrange2'])&&($unique_id > $row4['startrange2']))||(($unique_id <= $row4['lastrange3'])&&($unique_id > $row4['startrange3']))||(($unique_id <= $row4['lastrange4'])&&($unique_id > $row4['startrange4'])))
		//elseif($unique_id > $row4['datarange'])
		{
			//process the query
